...
RSpec.describe "Clients", type: :request do
  ...
  context "when the user is signed in" do
    ...
    describe "GET /clients/:id" do
      ...
      it "shows the client activity timeline" do
        client = create(:client, user: user)
        ...
        note = create(
          ...
          content: "Discussed the project requirements."
        )
        ...
        expect(response.body).to include("Task created")
        expect(response.body).to include(task.title)
        expect(response.body).to include("High priority")
        expect(response.body).to include("Due August 10, 2026")
      end

      it "shows task started and completed activity" do
        client = create(:client, user: user)
        task = create(
          :task,
          client: client,
        ...
        expect(response).to have_http_status(:success)
        expect(response.body).to include("Task started")
        expect(response.body).to include("Task completed")
        expect(response.body).to include(task.title)
      end

      it "shows the empty activity state when there are no saved tasks or notes" do
        client = create(:client, user: user)
